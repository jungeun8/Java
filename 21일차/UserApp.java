	...
	public static void main(String[] args) {
		...
		UserService service = new UserService();
			...
			while(true) {
				try {
				System.out.println("-------------------------------------");
				System.out.println("1. 가입하기 2. 비밀번호 변경 3. 회원탈퇴 4. 조회 0. 종료");
				System.out.println("-------------------------------------");
				...
				if (menuNo == 1) {
					...
					service.changUsesrPassword(id, oldPassword, newPassword);  // 해당 메소드가 던진 예외를 받으면 catch블록으로 점프 
					...
					User user = service.getuUserById(userid);
					if(user == null) {
